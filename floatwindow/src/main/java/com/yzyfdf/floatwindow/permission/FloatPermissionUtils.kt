...
import com.yzyfdf.floatwindow.permission.rom.*


object FloatPermissionUtils {

    internal const val requestCode = 199
    private const val TAG = "FloatPermissionUtils--->"
    ...
    internal fun requestPermission(fragment: Fragment) =
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.M) when {
            ...
            else -> LogUtils.i(TAG, "原生 Android 6.0 以下无需权限申请")
    ...
    private fun commonROMPermissionApply(fragment: Fragment) = when {
        ...
        // 需要做统计效果
        else -> LogUtils.d(TAG, "user manually refuse OVERLAY_PERMISSION")
    }
